...
BEGIN  
    ...
    ELSE
	CREATE temporary TABLE THELOAI AS
   SELECT * FROM ( food join food_category on food.ID=food_category.FID)
...
BEGIN  
   IF fprice ='THAP' THEN
    SELECT * FROM food where FOOD.PRICE>0 AND FOOD.PRICE<=50000;
   ELSEIF fprice='TRUNG BINH' THEN
    SELECT * FROM food where FOOD.PRICE>50000 AND FOOD.PRICE<=150000;
   ELSEIF fprice='CAO' THEN
    SELECT * FROM food where FOOD.PRICE>150000 AND FOOD.PRICE<=300000;
   ELSEIF fprice='DAC BIET' THEN 
    SELECT * FROM food where FOOD.PRICE>300000; 
    ELSE
    SELECT * FROM food;
   END IF;
END
